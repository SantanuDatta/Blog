            ...
            <div class="post-carousel-lg">
                ...
                @foreach (App\Models\Post::orderBy('id', 'desc')->where('is_featured', 1)->take(5)->get() as $lPost)
                    <div class="post featured-post-xl">
                        @php
                            $commentCount = DB::table('comments')->where('post_id', $lPost->id)->where('status', 1)->get();
                        @endphp
                        <div class="details clearfix">
                            ...
                            <h4 class="post-title"><a href="{{ route('singlePost', $lPost->slug) }}">{{ $lPost->title }}</a></h4>
                        ...
                        <a href="{{ route('singlePost', $lPost->slug) }}">
                            <div class="thumb rounded">
                                @if (!is_null($lPost->image))
                                    <div class="inner data-bg-image" data-bg-image=" {{ asset('backend/img/post-images/'.$lPost->image) }}"></div>
                                @else
                                    <div class="inner data-bg-image" data-bg-image=" {{ asset('backend/img/post-images/post_default.png') }}"></div>
                                @endif
							...
							<div class="col-sm-6">
                                ...
                                @foreach (App\Models\Post::orderBy('id', 'desc')->take(1)->get() as $post)
                                    <div class="post">
                                        ...
                                        <h5 class="post-title mb-3 mt-3"><a href="{{ route('singlePost', $post->slug) }}">{{ Str::limit($post->title, 50) }}</a></h5>
                                ...
                                <div class="post post-over-content col-md-6">
                                    <div class="details clearfix">
                                        @foreach (explode(",", $post->category_id) as $cat)
                                            @foreach (App\Models\Category::where('id', $cat)->get() as $allCat)
                                                <a href="{{ route('categoryPost', $allCat->slug) }}" class="category-badge">{{ $allCat->name }}</a>
                                            @endforeach
                                        @endforeach
                                        <h4 class="post-title"><a href="{{ route('singlePost', $postDetails->slug) }}">{{ Str::limit($postDetails->title, 40) }}</a></h4>
                                        <ul class="meta list-inline mb-0">
                                            <li class="list-inline-item">by {{ $postDetails->user->name }}</li>
                                            <li class="list-inline-item">{{ $postDetails->created_at->toFormattedDateString() }}</li>
                                            @php
                                                $commentCount = DB::table('comments')->where('post_id', $postDetails->id)->where('status', 1)->get();
                                            @endphp
                                            <li class="list-inline-item">{{ $commentCount->count() }} Comments </li>
                                        </ul>
                                    </div>
                                        ...
                                        <div class="thumb rounded">
                                            <div class="inner">
                                                @if (!is_null($postDetails->image))
                                                        <img src="{{ asset('backend/img/post-images/'.$postDetails->image) }}" alt="thumb" />
                                                    @else
                                                        <img src="{{ asset('backend/img/post-images/post_default.png') }}" alt="thumb" />
                                                    @endif
                                            </div>
                                        </div>
                                    </a>
