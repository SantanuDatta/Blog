<nav class="navbar navbar-expand-lg">
    <div class="container-xl">
        ...
        @foreach ($settings as $setting)
            <a class="navbar-brand" href="{{ route('home') }}"><img src="{{ asset('backend/img/settings/logo/'. $setting->logo) }}" alt="logo" /></a> 
        @endforeach
        ...
        <div class="collapse navbar-collapse">
            ...
            <ul class="navbar-nav mr-auto">
                ...
                <li class="nav-item @if (Route::is('blogs') || Route::is('singlePost'))
                    active
                @endif">
                    <a class="nav-link" href="{{ route('blogs') }}">Blogs</a>
                </li>
                <li class="nav-item dropdown @if (Route::is('categoryPost'))
                        active
                    @endif"">
                    ...
                    <ul class="dropdown-menu">
                        @foreach (App\Models\Category::where('is_parent', 0)->where('status', 1)->orderBY('name', 'asc')->get() as $pCat)
                                <li>
                                    <a class="dropdown-item" href="{{ route('categoryPost', $pCat->slug) }}">{{ $pCat->name }}</a>
                                </li>
                ...
                <li class="nav-item @if (Route::is('contact'))
                    active
                @endif">
